...
func TestCompileRejectsUnsafeOrInvalidSpec(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "padded absolute directory", spec: withSpec(base, func(s *verification.CommandSpec) { s.Directory = " " + filepath.Join(workspace, "nested") })},
		{name: "padded escaping directory", spec: withSpec(base, func(s *verification.CommandSpec) { s.Directory = " ../outside " })},
		{name: "normalized parent directory", spec: withSpec(base, func(s *verification.CommandSpec) { s.Directory = "safe/../other" })},
