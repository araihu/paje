...
func (target *Executor) readChildStartReceipt(ctx context.Context, containerID string) (executor.ChildStartReceipt, error) {
	...
	if err := decoder.Decode(&receipt); err != nil {
		return executor.ChildStartReceipt{}, errors.New("decode Docker child-start receipt")
	}
	var trailing any
	if err := decoder.Decode(&trailing); !errors.Is(err, io.EOF) {
		return executor.ChildStartReceipt{}, errors.New("Docker child-start receipt has trailing content")
	}
