...
func generate(out string) error {
	...
	if err := write(filepath.Join(out, "site.css"), []byte(style+brandStyle)); err != nil {
		return err
	}
...
<html lang="%s"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>%s · Pajé</title><meta name="description" content="%s"><meta property="og:title" content="%s"><meta property="og:description" content="%s"><meta property="og:image" content="/og.png"><meta name="twitter:card" content="summary_large_image"><link rel="canonical" href="/%s"><link rel="icon" href="/paje-icon-background.svg">%s<link rel="stylesheet" href="/site.css"></head>
<body><a class="skip" href="#main">Skip to content</a><header class="mast"><a class="brand" href="/%s" aria-label="Pajé home"><img src="/paje-logo-transparent.svg" alt="" width="166" height="41"></a><nav aria-label="Primary"><a href="#protocol">Protocol</a><a href="#boundary">Boundary</a><a href="https://github.com/araihu/paje">GitHub</a></nav><div class="languages" aria-label="Language"><a href="/en"%s>EN</a><a href="/pt-br"%s>PT</a><a href="/es"%s>ES</a></div></header>
...
</main><footer class="foot"><img src="/paje-icon-background.svg" alt="" width="26" height="26"><span>Pajé · durable workflow infrastructure</span><a href="https://github.com/araihu/paje">%s</a></footer></body></html>`,
		html.EscapeString(c.lang), html.EscapeString(c.title), html.EscapeString(c.description), html.EscapeString(c.title), html.EscapeString(c.description), c.locale, deps, c.locale,
...
const brandStyle = `
:root{color-scheme:light;--paper:#f3f2e9;--paper-deep:#e4e8eb;--ink:#07111f;--muted:#243348;--line:#a3b0bf;--blue:#173b72;--night:#07111f;--green:#258256;--signal:#c7ff4a}.brand img{display:block;width:166px;height:auto}.mast nav a,.quiet-link,.lead{color:var(--muted)}.languages a[aria-current]{color:var(--paper)}@media(prefers-color-scheme:dark){:root{color-scheme:dark;--paper:#07111f;--paper-deep:#10233d;--ink:#f7f8f3;--muted:#d5ddeb;--line:#526981;--blue:#c7ff4a;--night:#020812;--green:#79d59b}.run{background:var(--paper-deep);border-color:var(--line)}.run-head,.run footer,.run li{border-color:var(--line)}.run-head,.run li.active b{color:var(--ink)}}`
